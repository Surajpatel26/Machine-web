...
export default function About() {
  ...
  return (
    <div style={{ background: 'var(--bg-white)', minHeight: '100vh', overflow: 'hidden' }} className="pt-nav">
      
      {/* ── Progress Bar ── */}
      <motion.div style={{ scaleX, position: 'fixed', top: 64, left: 0, right: 0, height: '4px', background: 'var(--brand-blue)', zIndex: 100, originX: 0 }} />
      ...
      <section style={{ height: '90vh', position: 'relative', display: 'flex', alignItems: 'center' }}>
        ...
        <div className="max-w-wide" style={{ position: 'relative', zIndex: 1, padding: '0 2rem' }}>
          ...
          >
            ...
            <p className="f-body" style={{ fontSize: '1.25rem', color: 'var(--ink-mid)', maxWidth: '500px' }}>
      ...
      <section style={{ padding: '10rem 0' }}>
        <div className="max-w-wide" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '100px', padding: '0 2rem' }}>
          ...
          >
            <h2 className="f-display" style={{ fontSize: '3rem', marginBottom: '48px', lineHeight: 1.1 }}>
              Engineering <br />
              <span className="f-display-italic">Heritage</span>
            </h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
              <p className="f-body" style={{ fontSize: '1.1rem', color: 'var(--ink-mid)' }}>
              ...
              <p className="f-body" style={{ fontSize: '1.1rem', color: 'var(--ink-mid)' }}>
                What started as a specialized workshop has evolved into a powerhouse of precision engineering, serving over 500+ global clients with cutting-edge CNC and VMC solutions.
              </p>
            </div>
          </motion.div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
            {[
              ...
              { label: 'Precision Engineering', value: '25 Years' },
              ...
              <motion.div
                ...
                style={{ padding: '40px', background: '#fff', borderRadius: '24px', border: '1px solid rgba(0,0,0,0.05)', textAlign: 'center' }}
              >
                <div className="f-display" style={{ fontSize: '2.5rem', color: 'var(--brand-blue)', marginBottom: '8px' }}>{stat.value}</div>
                <div className="f-label" style={{ fontSize: '0.7rem', color: 'var(--ink-light)', textTransform: 'uppercase' }}>{stat.label}</div>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: '120px 0', background: 'var(--bg-white)', borderTop: '1px solid rgba(0,0,0,0.05)' }}>
            ...
            {values.map((v, i) => (
              <motion.div
                ...
                style={{ 
                  ...
                  gap: '32px',
                  paddingTop: i % 2 !== 0 ? '60px' : '0'
                }}
              >
                <div style={{ 
                  fontFamily: "'Playfair Display', serif", 
                  fontSize: '5rem', 
                  color: 'rgba(59,107,149,0.08)', 
                  lineHeight: 1,
                  fontWeight: 900,
                  fontStyle: 'italic'
                }}>
                ...
                <div style={{ paddingLeft: '20px', borderLeft: '1px solid var(--brand-gold)' }}>
                  <h3 className="f-display" style={{ fontSize: '1.75rem', marginBottom: '20px', color: 'var(--ink)' }}>{v.title}</h3>
                  <p className="f-body" style={{ color: 'var(--ink-mid)', fontSize: '1.05rem', lineHeight: 1.6 }}>{v.desc}</p>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      {/* ── Timeline Section ── */}
      <section style={{ padding: '10rem 0' }}>
        <div className="max-w-text" style={{ margin: '0 auto', padding: '0 2rem' }}>
          <div style={{ textAlign: 'center', marginBottom: '80px' }}>
            <h2 className="f-display" style={{ fontSize: '3.5rem' }}>Our <span className="f-display-italic">Evolution</span></h2>
          </div>
          <div style={{ position: 'relative' }}>
            <div style={{ position: 'absolute', left: '20px', top: 0, bottom: 0, width: '1px', background: 'rgba(0,0,0,0.1)' }} />
            <div style={{ display: 'flex', flexDirection: 'column', gap: '64px' }}>
              {timeline.map((item, i) => (
                ...
                >
                  ...
                  <div className="f-display" style={{ fontSize: '2rem', color: 'var(--brand-gold)', marginBottom: '4px' }}>{item.year}</div>
                  <h4 className="f-label" style={{ fontSize: '0.8rem', color: 'var(--ink)', marginBottom: '12px', letterSpacing: '0.1em' }}>{item.title}</h4>
                  <p className="f-body" style={{ color: 'var(--ink-mid)' }}>{item.desc}</p>
                </motion.div>
              ))}
            </div>
          </div>
      ...
      {/* ── Closing CTA ── */}
      <section style={{ padding: '120px 0', textAlign: 'center', background: 'var(--brand-blue)' }}>
        <div className="max-w-text" style={{ margin: '0 auto', padding: '0 2rem' }}>
          <h2 className="f-display" style={{ color: '#fff', fontSize: '3.5rem', lineHeight: 1.1, marginBottom: '32px' }}>
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="btn-pill"
            style={{ background: '#fff', color: 'var(--brand-blue)', padding: '20px 48px', fontSize: '1rem', fontWeight: 600 }}
          >
            Contact Our Experts
          </motion.button>
        </div>
